...
export default class RoomClient
{
	...
	)
	{
		...
		// Custom media-client handler name (to override default browser
		...
		// media-client Device instance.
		// @type {mediaClient.Device}
		this._mediasoupDevice = null;

		// media Transport for sending.
		// @type {mediaClient.Transport}
		this._sendTransport = null;

		// media Transport for receiving.
		// @type {mediaClient.Transport}
		this._recvTransport = null;

		// Local mic media Producer.
		// @type {mediaClient.Producer}
		this._micProducer = null;

		// Local webcam media Producer.
		// @type {mediaClient.Producer}
		this._webcamProducer = null;

		// Local share media Producer.
		// @type {mediaClient.Producer}
		this._shareProducer = null;

		// Local chat DataProducer.
		// @type {mediaClient.DataProducer}
		...
		// media Consumers.
		// @type {Map<String, mediaClient.Consumer>}
		this._consumers = new Map();

		// media DataConsumers.
		// @type {Map<String, mediaClient.DataConsumer>}
				...
				case 'newConsumer':
				{
					...
					try
					{
						...
						const { spatialLayers, temporalLayers } =
							mediaClient.parseScalabilityMode(
	...
	async _joinRoom()
	{
		...
		try
		{
			this._mediasoupDevice = new mediaClient.Device(
				{
			...
			// Create media Transport for sending (unless we don't want to produce).
			if (this._produce)
			{
			...
			// Create media Transport for receiving (unless we don't want to consume).
			if (this._consume)
			{
				const transportInfo = await this._protoo.request(
					'createWebRtcTransport',
